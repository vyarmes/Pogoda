...
async function getWeatherByCoords(lat, lon) {
  ...
  try {
    ...
    let name = 'Місце', country = '', admin1 = '', population = 0;
    if (revGeoRes.ok) {
      const revGeo = await revGeoRes.json();
      name = revGeo.address?.city || revGeo.address?.town || revGeo.address?.village || 'Місце';
      country = revGeo.address?.country || '';
    }
    ...
    displayChart(wData);
    displayAstronomy(wData);
    drawWindRose(wData);
    showMap(lat, lon, name);
  } catch(err) {
    console.error(err);
    showError('❌ Помилка при отриманні погоди. Спробуйте іншу точку.');
  }
}
...
function useGeolocation() {
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    err => {
      ...
      showError('❌ Не вдалося отримати вашу геолокацію. Перевірте дозволи браузера');
      btn.classList.remove('loading');
      btn.textContent = '📍';
    }
  );
...
function getMoonPhase(date) {
  const phases = ['🌑 Новий', '🌒 Молодик', '🌓 Перша чверть', '🌔 Першої половини', 
                  '🌕 Повня', '🌖 Останньої половини', '🌗 Остання чверть', '🌘 Старик'];
  const d = new Date(date);
  const knownNewMoon = new Date(2000, 0, 6);
  const lunaCycle = 29.53;
  const diff = (d - knownNewMoon) / (1000 * 60 * 60 * 24);
  const phase = Math.floor((diff % lunaCycle) / (lunaCycle / 8));
  return phases[Math.max(0, Math.min(7, phase))];
}

function displayAstronomy(data) {
  const daily = data.daily;
  const today = 0;
  const sunrise = daily.sunrise[today].split('T')[1];
  const sunset = daily.sunset[today].split('T')[1];
  const date = daily.time[today];
  
  const html = `
    <div class="astro-item">
      <div class="astro-label">Рассвіт</div>
      <div class="astro-value">🌅</div>
      <div class="astro-desc">${sunrise}</div>
    </div>
    <div class="astro-item">
      <div class="astro-label">Захід</div>
      <div class="astro-value">🌄</div>
      <div class="astro-desc">${sunset}</div>
    </div>
    <div class="astro-item">
      <div class="astro-label">Місяць</div>
      <div class="astro-value">${getMoonPhase(date).split(' ')[0]}</div>
      <div class="astro-desc">${getMoonPhase(date).split(' ')[1]}</div>
    </div>
    <div class="astro-item">
      <div class="astro-label">Видимість</div>
      <div class="astro-value">⭐</div>
      <div class="astro-desc">Добра</div>
    </div>
  `;
  $('astronomy-data').innerHTML = html;
  $('astronomy-section').style.display = 'block';
}

/* ── Wind Rose / Wind Direction Chart ────────────────– */
function drawWindRose(data) {
  ...
  // Draw wind bars for each day
  const windSpeeds = data.daily?.wind_speed_10m_max;
  
  if (!windSpeeds) return;
  ...
  for (let i = 0; i < Math.min(8, windSpeeds.length); i++) {
    const speed = windSpeeds[i];
    const dir = currentWindDir + (i * 5);
    const angle = dir * Math.PI / 180 - Math.PI / 2;
    const len = (speed / 30) * radius;
  ...
  $('wind-rose-section').style.display = 'block';
}
